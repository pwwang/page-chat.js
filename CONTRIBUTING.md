# Contributing to PageChat

> **[A note from the maintainer](https://github.com/alibaba/page-chat/issues/349)** — Please read before opening issues or PRs.
...
- **Core** (`packages/core/`) - Core agent logic without UI (npm: `@page-chat/core`)
- **Extension** (`packages/extension/`) - Chrome extension for multi-page tasks and browser-level automation
- **Website** (`packages/website/`) - React documentation and landing page. Also as demo and test page for the core lib. private package `@page-chat/website`
...
1. **Fork and Clone**
    ...
    ```bash
    git clone https://github.com/your-username/page-chat.git
    cd page-chat
    ```
...
- **Ollama example** (tested on 0.15 + qwen3:14b, RTX3090 24GB):
    ...
    > @see https://alibaba.github.io/page-chat/docs/features/models#ollama for configuration

- **Restart the dev server** to load new env vars
- If not provided, the demo will use the free testing proxy by default. By using it, you agree to its [terms](https://github.com/alibaba/page-chat/blob/main/docs/terms-and-privacy.md).
...
npm run dev -w @page-chat/ext
npm run zip -w @page-chat/ext
```
...
- Start and serve a local `iife` script
    ...
    ```bash
    npm run dev:demo # Serving IIFE with auto rebuild at http://localhost:5174/page-chat.demo.js
    ```

- Add a new bookmark

    ```javascript
    javascript:(function(){var s=document.createElement('script');s.src=`http://localhost:5174/page-chat.demo.js?t=${Math.random()}`;s.onload=()=>console.log(%27PageChat ready!%27);document.head.appendChild(s);})();
    ```

- Click the bookmark on any page to load Page-Chat
...
Thank you for helping make PageChat better! 🎉
